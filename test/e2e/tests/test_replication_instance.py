...
* test_crud
    ...
    tags, update tags, delete tags, update a simple field
    (autoMinorVersionUpgrade), toggle the``multiAZ`` flag and let the fixture
    handle deletion.
"""
...
@pytest.fixture
def replication_instance(request):
    ...
    yield ri_ref, ri_cr, instance_name
...
class TestReplicationInstance:
    def test_crud(self, replication_instance):
        ...
        5.  ACK system tags are present in the AWS API before user tags are
            applied.
        6.  User tags can be added as ``environment=dev``, updated to
            ``environment=prod``, and then removed.
        ...
        """
        ref, cr, instance_name = replication_instance
        ...
        assert cr is not None
        assert cr['status']['instanceStatus'] == 'available'
        instance_arn = k8s.get_resource_arn(cr)
        assert instance_arn is not None

        # ---- Verify initial tags --------------------------------------------
        latest_tags = aws_api.get_tags(instance_arn)
        ...
        # ---- Update: autoMinorVersionUpgrade --------------------------------
        ...
        # ---- Confirm MultiAZ is False ---------------------------------------
        assert latest is not None
        assert latest['MultiAZ'] is False

        # ---- Update: MultiAZ ------------------------------------------------
        k8s.patch_custom_resource(ref, {"spec": {"multiAZ": True}})

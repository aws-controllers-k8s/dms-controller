...
func (rm *resourceManager) syncTags(
	...
	if len(toDelete) > 0 {
		rlog.Debug("removing tags from certificate", "tags", toDelete)
	...
	if len(toAdd) > 0 {
		rlog.Debug("adding tags to certificate", "tags", toAdd)
...
// customSdkUpdate supports Tag updates for Certificates.
//
// DMS does not provide a ModifyCertificate API, so we have to use a custom
// method to update Tags.
func (rm *resourceManager) customSdkUpdate(
	ctx context.Context,
	desired *resource,
	latest *resource,
	delta *ackcompare.Delta,
	...
	rlog := ackrtlog.FromContext(ctx)
	exit := rlog.Trace("rm.customSdkUpdate")
	defer func() {
		exit(err)
	}()
	...
	if delta.DifferentExcept("Spec.Tags") {
		return nil, ackerr.NewTerminalError(
			fmt.Errorf("updating fields other than spec.tags is not supported"),
		)
	}

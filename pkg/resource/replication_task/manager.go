...
// +kubebuilder:rbac:groups=dms.services.k8s.aws,resources=replicationtasks,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dms.services.k8s.aws,resources=replicationtasks/status,verbs=get;update;patch
...
func (rm *resourceManager) IsSynced(ctx context.Context, res acktypes.AWSResource) (bool, error) {
	...
	if r.ko.Status.TaskStatus == nil {
		return false, nil
	}
	taskStatusCandidates := []string{"failed", "ready", "running", "stopped"}
	if !ackutil.InStrings(*r.ko.Status.TaskStatus, taskStatusCandidates) {
		return false, nil
	}

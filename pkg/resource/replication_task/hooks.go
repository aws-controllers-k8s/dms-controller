...
// indentJson is a helper function that takes a pointer to a
// JSON string, indents it, and returns a pointer to the indented
// JSON string.
func indentJson(src *string) (*string, error) {
	if src == nil {
		return nil, nil
	}
...
// ensureJsonIndented is a helper function that ensures the
// JSON string fields in the ReplicationTask Spec are indented
// for better readability.
func ensureJsonIndented(ko *svcapitypes.ReplicationTask) (err error) {
	if ko.Spec.TableMappings != nil {
		indented, err := indentJson(ko.Spec.TableMappings)
		if err != nil {
			return err
		}
		ko.Spec.TableMappings = indented
	}
	if ko.Spec.TaskSettings != nil {
		indented, err := indentJson(ko.Spec.TaskSettings)
		if err != nil {
			return err
		}
		ko.Spec.TaskSettings = indented
	}
	if ko.Spec.TaskData != nil {
		indented, err := indentJson(ko.Spec.TaskData)
		if err != nil {
			return err
		}
		ko.Spec.TaskData = indented
	}
	return nil
}
